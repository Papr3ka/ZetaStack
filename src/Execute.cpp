		...
		*/	
		...
		while(index < stopsize){
			...
			case 4:{
				...
				if(!(fexists(funcname, farg_max) || funcname == freserved || f_isspecial(funcname, farg_max))){
				...
				if(dumpfunction || (funcname == freserved && farg_max == 0)){
					dumpfunction = false;
					evalstack.emplace_back(tokens[index]);
					index++;
					continue;
		...
		*/	
		...
		while(index < stopsize){
			...
			case 4:{ 
				std::string funcname = tokens[index].data;
				farg_max = tokens[index].reserved;
				if(!(fexists(funcname, farg_max) || f_isspecial(funcname, farg_max))){
					std::string error = "No matching function call to \"";
					error.append(funcname).append("\"");
					throw error;
				}
		...
		if(evalstack.size() <= 0){
			...
			// Success
			std::string strout = evalstack.front().data;
